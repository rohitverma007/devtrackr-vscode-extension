...
async function activate(context) {
  try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("devtrackr.refreshEntry", async function () {
      await dataProvider.refresh();
      vscode.commands.executeCommand("workbench.action.reloadExtension");
    }));
    context.subscriptions.push(vscode.commands.registerCommand("devtrackr.deleteProject", async ({ label }) => {
    ...
    vscode.commands.registerCommand("devtrackr.save", async () => {
      ...
      if (!Object.keys(dataProvider.projects).length) {
        return vscode.window.showInformationMessage(`Please add project then click on save button`);
      }
      if (!(await githubProvider.getSecrets())) {
        return vscode.window.showErrorMessage(`Please set your GitHub personal access token and username by clicking on "Github Config" Button`);
      }
      await githubProvider.processProjects(dataProvider);
    });
    context.subscriptions.push(vscode.commands.registerCommand("devtrackr.addProject", async function () {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("devtrackr.storeGithubToken", async () => {
      ...
      if (username && token) {
        ...
        vscode.window.showInformationMessage(`Please don't leave it github username or token empty`);
      }
    }));
